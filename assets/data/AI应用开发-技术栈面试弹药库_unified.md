...
### Q: 为什么要可逆加密？bcrypt 不行吗？

PAT 令牌安全存储 — 为什么要可逆加密？bcrypt 不行吗？

**答案**：

业务场景：用户发布工作流为 API 后，外部系统通过 Personal Access Token 调用。后端收到令牌后需要解密出原始 token，以用户身份代理调用 Coze API——这是代理模式，不是认证比对模式。
...
### Q: 为什么需要压缩？直接存 JSON 不行吗？

执行快照压缩存储 — 为什么需要压缩？直接存 JSON 不行吗？
...
### Q: MongoDB 部分唯一索引 & TTL 索引还用在哪些场景？

幂等性控制 — MongoDB 部分唯一索引 & TTL 索引还用在哪些场景？
...
### Q: Coze、LangChain、LlamaIndex 区别？怎么选？

Coze 引擎集成 — Coze、LangChain、LlamaIndex 区别？怎么选？
...
### Q: IntelligentSystem 为什么不直接调用自研 rag 服务？

Coze 引擎集成 — IntelligentSystem 为什么不直接调用自研 rag 服务？
...
### Q: 固定窗口 vs 滑动窗口，为什么选前者？

多租户固定窗口限流器 — 固定窗口 vs 滑动窗口，为什么选前者？
...
### Q: 五种 Redis 基础数据结构？用了哪个？

多租户固定窗口限流器 — 五种 Redis 基础数据结构？用了哪个？
...
BGE-M3 / Chinese-CLIP 的 OpenAI 兼容 API 层 — 为什么需要兼容层？
...
### Q: dimensions 参数的边界情况

BGE-M3 / Chinese-CLIP 的 OpenAI 兼容 API 层 — dimensions 参数的边界情况
...
### Q: BGE-M3 为什么一个模型输出三种向量？

BGE-M3 / Chinese-CLIP 的 OpenAI 兼容 API 层 — BGE-M3 为什么一个模型输出三种向量？
...
BGE-M3 / Chinese-CLIP 的 OpenAI 兼容 API 层 — SSRF 为什么是安全问题？防什么？
...
### Q: Rerank 为什么需要？直接向量检索不行吗？

检索评估框架与端到端验证 — Rerank 为什么需要？直接向量检索不行吗？
...
### Q: 为什么用 Elasticsearch 而不是 Milvus/Pinecone？

检索引擎架构 — 为什么用 Elasticsearch 而不是 Milvus/Pinecone？
...
### Q: 为什么封装 LlamaIndex 而不是直接用？

检索引擎架构 — 为什么封装 LlamaIndex 而不是直接用？
...
### Q: async def vs def 路由函数？

FastAPI + Pydantic — async def vs def 路由函数？
...
FastAPI + Pydantic — Pydantic v2 vs v1？
...
### Q: 什么时候用 MongoDB 而不是关系型？

MongoDB — 什么时候用 MongoDB 而不是关系型？
...
### Q: 倒排索引为什么比 MySQL LIKE 快？

Elasticsearch — 倒排索引为什么比 MySQL LIKE 快？
...
### Q: BM25 relevance score 怎么算？

Elasticsearch — BM25 relevance score 怎么算？
...
### Q: Router → Service → Repository 三层是不是过度设计？

工程通用 — Router → Service → Repository 三层是不是过度设计？
...
### Q: 异步任务为什么用 Celery 而不是 ThreadPoolExecutor？

工程通用 — 异步任务为什么用 Celery 而不是 ThreadPoolExecutor？
...
### Q: 检索链路为什么用 ThreadPoolExecutor 而不是 async/await？

工程通用 — 检索链路为什么用 ThreadPoolExecutor 而不是 async/await？
...
RAG 评测平台 — 可插拔引擎与 LLM-as-Judge — 评测平台解决什么问题？
...
RAG 评测平台 — 可插拔引擎与 LLM-as-Judge — 可插拔引擎架构怎么设计的？
...
RAG 评测平台 — 可插拔引擎与 LLM-as-Judge — IR 引擎的三种匹配模式是什么？
...
### Q: Ragas 引擎的离线启发式模式是什么？

RAG 评测平台 — 可插拔引擎与 LLM-as-Judge — Ragas 引擎的离线启发式模式是什么？
...
### Q: LLM-as-Judge 的评判结果可信吗？

RAG 评测平台 — 可插拔引擎与 LLM-as-Judge — LLM-as-Judge 的评判结果可信吗？
...
### Q: 评测任务为什么用 Celery 异步执行？

RAG 评测平台 — 可插拔引擎与 LLM-as-Judge — 评测任务为什么用 Celery 异步执行？
...
Embedding 模型推理服务 — 多模型 GPU 托管 — 为什么一个进程里加载三个模型？
...
Embedding 模型推理服务 — 多模型 GPU 托管 — 信号量并发控制怎么设计的？
...
### Q: BGE-M3 为什么同时输出三种向量？各用在哪？

Embedding 模型推理服务 — 多模型 GPU 托管 — BGE-M3 为什么同时输出三种向量？各用在哪？
...
### Q: Chinese-CLIP 的图文共享语义空间是什么意思？

Embedding 模型推理服务 — 多模型 GPU 托管 — Chinese-CLIP 的图文共享语义空间是什么意思？
...
Embedding 模型推理服务 — 多模型 GPU 托管 — SSRF 防护为什么重要？
...
文档摄入流水线 — 从上传到可检索 — 五阶段异步流水线的每个阶段做什么？
...
### Q: Docx 引擎的 OCR 调用为什么用 ThreadPoolExecutor？

文档摄入流水线 — 从上传到可检索 — Docx 引擎的 OCR 调用为什么用 ThreadPoolExecutor？
...
### Q: Rerank 为什么放最后而不是放 fusion 之前？

文档摄入流水线 — 从上传到可检索 — Rerank 为什么放最后而不是放 fusion 之前？
...
### Q: ES 索引的 mapping 设计有什么讲究？

文档摄入流水线 — 从上传到可检索 — ES 索引的 mapping 设计有什么讲究？
...
### Q: 每个知识库为什么强制绑定模型且不可变？

全局模型目录 — 统一接入层 — 每个知识库为什么强制绑定模型且不可变？
...
### Q: kNN 的 numcandidates 为什么设为 candidatek × 4？

Elasticsearch — kNN 的 num_candidates 为什么设为 candidate_k × 4？
